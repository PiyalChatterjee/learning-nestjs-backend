...
### Symptom
- Course used AWS S3 + CloudFront; user had no paid AWS account and needed an equivalent

### Root Cause
- N/A — intentional architectural substitution, not an error

### Change Made
- Created `src/modules/uploads/` with full module structure: controller, service, entity, DTO
- Used `@azure/storage-blob` SDK with `BlobServiceClient.fromConnectionString()` for Azure uploads
- Used `crypto.randomUUID()` instead of `uuid` package to avoid ESM/CommonJS conflicts
- Typed Multer files with local `IUploadFile` interface instead of `Express.Multer.File` to avoid type hierarchy conflicts
- Azure credentials stored in `.env` and read via `ConfigService` at `appConfig.azureStorage.connectionString`
- Upload endpoints marked `@Auth(AuthType.None)` to bypass global `AuthenticationGuard`
- httpyac test files required explicit RFC multipart boundary format (not `@form-data` shorthand)
- Azure Front Door CDN endpoint integrated through `AZURE_CDN_ENDPOINT` with endpoint normalization and safe fallback to direct blob URL

### Verification
- Both `POST /v1/uploads/file` and `POST /v1/uploads/files` tested via httpyac
- Files visible in Azure Portal: `azurelearning02354594800` storage account → `azureml` container
- Two blobs confirmed: `.png` (1.15 KiB) and `.txt` (30 B)
- API upload response verified to return Azure Front Door URL host after CDN deployment

### Deferred
- Automated tests for uploads module and CDN fallback behavior are intentionally postponed to a later testing pass.

### Lesson/Topic Context
- `multipart/form-data` boundary must be auto-generated by the HTTP client — manually setting `Content-Type: multipart/form-data` without a boundary causes "Boundary not found" errors
- Each file in a multi-file upload needs its own `--boundary` part block with its own `Content-Disposition` header
