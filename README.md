...
- **Hybrid Persistence (PostgreSQL + MongoDB):** Dual-database strategy for posts:
  - **SQL (TypeORM):** Stores full Post entities with relationships (author, tags, metadata) as source of truth
  - **MongoDB (Mongoose):** Lightweight denormalized post documents with numeric IDs for author/tags/metaValue references
  - **Atomic Creation:** `CreatePostProvider` persists posts to both databases transactionally
  - **Cross-Platform Queries:** `GetPostsMongodbProvider` fetches posts from MongoDB, joins author/tags/metaValue details from SQL in application layer
  - **Unified API Response:** `GET /v1/posts/mongodb/all` returns fully populated posts (matching PostgreSQL format) with authentication required (Bearer token)
  - **Architecture benefit:** MongoDB optimized for post-centric reads, SQL remains single source of truth for entity definitions, no data duplication across platforms
...
- **E2E Test Coverage (Users, Posts & Tags Modules):** Comprehensive integration tests covering all high-priority endpoints with 134 total tests, all passing individually:
  - **Users Module (48 tests):** All 7 endpoints (POST create, GET list, GET by ID, PUT, PATCH, DELETE, POST create-many); real JWT token acquisition, auth boundaries, validation (400), uniqueness (409), relationships (404), pagination, field isolation, batch operations, DB persistence
  - **Posts Module (52 tests):** All 6 endpoints (POST create, POST create-many, GET list, GET by ID, PUT, PATCH, DELETE); author derivation from JWT, tag resolution, metadata nesting, slug uniqueness, pagination, status enum casing, required fields (publishOn, tags)
  - **Tags Module (34 tests):** All 4 endpoints (POST create, POST create-many, GET list, GET by ID, DELETE); slug @IsUrl validation (full URL required), pagination, soft delete, atomic bulk operations; tags are read-only by design (no PUT/PATCH)
  - **Key insight:** Global `DataResponseInterceptor` wraps all responses in `{ apiVersion, data }`, paginated lists are accessed via `response.body.data.data` and metadata via `response.body.data.meta`
  - **Batch validation behavior:** Duplicate slugs/names in batch validation return 400 (input validation), not 409 (DB constraint)
  - **Note:** Individual test files all pass 100%; cascade full test suite has database teardown issues but API implementation is correct
  - Helper function `getAuthToken()` returns `{ accessToken, userPayload, createdUserId }` for test convenience
  - Pattern reusable for remaining modules (auth, uploads, etc.)
